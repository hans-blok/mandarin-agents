...
## Parameters
  ...
  - `agent_naam`: leveranciers-verkenner
  - `agent`: capability-architect
  - `value_stream_fase`: miv.07
  - `korte_beschrijving`: *Rol**   Verkent de markt van relevante hosting-providers en bouwt een onderbouwde longlist.  **Werkt op basis van**  - behoefteprofiel - selectiecriteria - afbakening van scope, zoals managed hosting, applicatiebeheer, private GitLab, EU-context en support tijdens kantooruren  **Belangrijkste taak**  - identificeert leveranciers die inhoudelijk in aanmerking komen - filtert partijen die evident niet passen bij de operationele context - maakt zichtbaar welke leveranciers waarschijnlijk sterk zijn in managed services, compliance, GitLab-beheer, supportmodel en schaalbaarheid  **Output**  - longlist met kandidaat-leveranciers - korte fit-notitie per leverancier - expliciete uitsluitingsgronden  **Boundary**  - doet nog geen eindoordeel of ranking - verandert geen eisenpakket
