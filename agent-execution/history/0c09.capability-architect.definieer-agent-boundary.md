...
## Parameters
  ...
  - `korte_beschrijving`: De Documentvertaler is verantwoordelijk voor het deterministisch omzetten  van een Markdown-document naar een correct en professioneel Word-document (.docx),  zonder wijziging van betekenis.  Hij realiseert representatie-omvorming, geen inhoudelijke interpretatie.  ## Primaire verantwoordelijkheid  Markdown → Word-conversie uitvoeren waarbij:  - betekenis volledig behouden blijft; - Markdown-specifieke syntactische artefacten worden genormaliseerd; - de output geschikt is voor professioneel gebruik.
  - `agent`: capability-architect
  - `vs`: fnd
  - `value_stream`: fnd
  - `fase`: 02
