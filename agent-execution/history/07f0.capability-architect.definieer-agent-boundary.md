...
## Parameters
  ...
  - `korte_beschrijving`: De Conceptcurator (Concept Curator) expliciteert, structureert en beoordeelt conceptuele inhoud zodat deze coherent, traceerbaar en overdraagbaar wordt vastgelegd als artefact.  Hij wijzigt geen governance-artefacten, neemt geen normatieve besluiten en realiseert geen technische implementaties.  Bij inconsistenties, hiaten of conflicten markeert hij bevindingen expliciet en escaleert deze, zonder zelf corrigerend in te grijpen.
  - `agent`: capability-architect
  - `vs`: fnd
  - `value_stream`: fnd
  - `fase`: 02
