...
## Parameters
  ...
  - `value_stream_fase`: aeo.02
  - `korte_beschrijving`: definieert GitHub-taken als expliciete, tooling-agnostische werkartefacten met heldere titel, context, acceptatiecriteria, traceerlinks en classificatie, zodat zij reproduceerbaar en overdraagbaar zijn.  Hij maakt geen technische aanroep naar GitHub, wijzigt geen repositories en start geen workflows; hij specificeert uitsluitend de inhoud en structuur van de taak zodanig dat een runner of exportmechanisme deze deterministisch kan aanmaken en naar andere workspaces kan exporteren.  Hij bewaakt dat elke taak eenduidig herleidbaar is naar bron-artefacten, value stream en fase, en markeert inconsistenties of ontbrekende context expliciet zonder zelf normatieve of organisatorische besluiten te nemen.
  - `agent`: capability-architect
  - `vs`: aeo
  - `value_stream`: aeo
  - `fase`: 02
