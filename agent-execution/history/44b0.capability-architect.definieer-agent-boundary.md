...
## Parameters
  ...
  - `value_stream_fase`: aeo.02
  - `korte_beschrijving`: Kernverantwoordelijkheid  De Agent-ontwerper constitueert de identiteit van een agent door het expliciet vastleggen van:  template, agent-contract  agent-charter  positionering binnen value stream en classificatie  Hij legitimeert wat de agent mag zijn.  Niet wat hij technisch doet.
  - `agent`: capability-architect
  - `vs`: aeo
  - `value_stream`: aeo
  - `fase`: 02
