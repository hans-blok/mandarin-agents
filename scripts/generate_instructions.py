...
Voorbeelden:
  ...
  python generate_instructions.py .github/prompts/mandarin.agent-smeder.leg-vast-agent-prompt.prompt.md -p agent=capability-architect
    ...
    parser.add_argument(
        "--minimal",
        action="store_true",
        help="Gebruik minimale instructieflow (sneller, minder context), bedoeld voor boundary-definitie"
    )
    parser.add_argument(
        "--minimal-template",
        type=str,
        default="temp/llm-minimal.md",
        help="Pad naar minimal prompt-template (default: temp/llm-minimal.md)"
    )
    parser.add_argument(
        "--skip-bootstrap",
        action="store_true",
        help="Sla verplichte canon-bootstrap over (alleen voor snelle iteraties/experimenteel gebruik)"
    )
    parser.add_argument(
        "--log-mode",
        type=str,
        ...
        help="Logging detailniveau voor audit/agent-instructions.log.md (default: full)"
    )
    parser.add_argument(
        "--bootstrap-quiet",
        action="store_true",
        help="Gebruik compacte bootstrap-output (minder terminal spam)"
    )
...
def load_minimal_template(template_path):
    """Laad minimal template content (frontmatter wordt gestript)."""
...
def run_bootstrap(metadata, prompt_file, method, params, workspace_config, quiet=False, charter_path=None, template_paths=None):
    ...
    # Los bootstrap script-pad op vanuit deze repository (mandarin-agents),
    # zodat uitvoering vanuit andere workspaces ook werkt.
    script_path = Path(script)
    if not script_path.is_absolute():
        repo_root = Path(__file__).resolve().parent.parent
        repo_relative = (repo_root / script_path).resolve()
        cwd_relative = (Path.cwd() / script_path).resolve()

        if repo_relative.exists():
            script_path = repo_relative
        elif cwd_relative.exists():
            script_path = cwd_relative
        else:
            print(f"ERROR: Bootstrap script niet gevonden: {script}")
            print(f"  - Geprobeerd (repo): {repo_relative}")
            print(f"  - Geprobeerd (cwd):  {cwd_relative}")
            sys.exit(1)
...
def replace_placeholders(content, params, input_content, input_files_content=None):
    ...
    for key, value in params.items():
        # Vervang [key] en [KEY] met lambda om regex escape issues te voorkomen (zoals backslashes in paden)
...
def write_execution_file(filepath, metadata, params, final_prompt):
    ...
    # Verplaats bestaande execution files naar history vóór het schrijven van een nieuw bestand.
    # Dit houdt agent-execution schoon zodat het nieuwste bestand direct selecteerbaar is.
    ...
    # Ondersteun zowel uitvoering in deze repository als vanuit een consumer-workspace.
    canon_hash = None
    repo_root = Path(__file__).resolve().parent.parent
    canon_log_candidates = [
        (repo_root / 'audit/canon-consult.log.md').resolve(),
        (Path.cwd() / 'audit/canon-consult.log.md').resolve(),
    ]
    # Deduplicate en kies primair het meest recent gewijzigde bestaande logbestand.
    unique_candidates = []
    for candidate in canon_log_candidates:
...
def log_agent_instructions(metadata, params, final_prompt, prompt_file, log_mode="full"):
...
def main():
    ...
    if args.agent and args.intent:
        ...
        # Boundary wordt lazy bepaald (alleen wanneer echt nodig)
        boundary_file = None
        ...
        # Probeer eerst value stream fase uit prompt metadata te halen
        try:
        ...
        # 2. Extraheer vs en fase uit prompt/boundary (alleen indien nodig)
        ...
        if 'value_stream_fase' in params and params['value_stream_fase']:
            ...
            if 'vs' not in params or not params.get('vs'):
                ...
                if not boundary_file:
                    print(f"  ⚠ Geen executor boundary gevonden voor '{args.agent}', ga door zonder boundary metadata")
                    print()
    ...
    if args.minimal:
        print("[Info] Minimal mode actief: gebruik compacte template en sla charter/agent-samenvoeging over.")
        minimal_content = load_minimal_template(args.minimal_template)
