...
Typical usage (from another workspace):
    ...
    python ..\\mandarin-agents\\scripts\\fetch_prompts.py fnd.02 --agent concept-curator --source ..\\mandarin-agents --target .
...
def find_agents(manifest: Dict, code: str, fase: str, agent: str | None = None) -> List[str]:
    for vs in manifest.get("value_streams", []):
        ...
        for fase_entry in vs.get("fasen", []):
            if str(fase_entry.get("volgnummer", "")).zfill(2) == fase:
                agents = [entry.get("naam", "").strip() for entry in fase_entry.get("agents", []) if entry.get("naam")]
                if agent:
...
def main(argv: List[str]) -> int:
    ...
    agents = find_agents(manifest, code, fase, args.agent)
    if args.agent and not agents:
        print(f"Geen prompts-agent match voor {args.agent} in {code}.{fase}")
        return 0
