...
def count_agent_artifacts(agent_path: Path) -> Dict[str, int] | None:
    ...
    Alleen agents MET subfolders agent-contracten/ EN (prompts/ OF templates/) worden geteld.
    Dit zijn volwaardige agents (niet legacy of incomplete).
    ...
    Returns:
        ...
        - aantal_templates: *.template.md bestanden in templates/
    ...
    # Een agent is geldig als hij contracten heeft EN (prompts OF templates)
    if not agent_contracten_path.exists() or (not prompts_path.exists() and not templates_path.exists()):
...
def build_json_structure(data: Dict[str, Dict[str, List[Dict]]]) -> Dict:
    ...
    Nu met agents_ref (HEAD hash) in de metadata header.
    """
    value_streams = []
    total_agents = 0

    # Haal HEAD hash op voor agents_ref
    import subprocess
    try:
        agents_ref = subprocess.check_output([
            'git', 'rev-parse', '--short', 'HEAD'
        ], cwd=str(Path(__file__).parent.parent), encoding='utf-8').strip()
    except Exception:
        agents_ref = None
    ...
    if agents_ref:
        metadata['agents_ref'] = agents_ref
