# Agents Publicatie Overzicht

**Publicatiedatum**: 2026-01-30
**Tijdstip**: 20:08:02
**Digest**: 84fe2
**Scope**: volledig
**Totaal agents**: 28

## Value Stream: agent-enablement

**Aantal agents**: 3

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| agent-curator | Agent boundary-setting, value stream administratie, agent ecosysteem oversight | 4 | 1 | 0 |
| agent-smeder | Agent-ontwerp, capability boundaries en contract-first uitvoering | 3 | 0 | 0 |
| template-maker | template-generatie op basis van charters | 2 | 0 | 0 |

## Value Stream: architectuur-en-oplossingsontwerp

**Aantal agents**: 5

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| archimate-modelleur | Enterprise architecture modellering | 2 | 0 | 0 |
| architectuur-regisseur | Architectuurdiscipline en besluitkwaliteit | 3 | 0 | 0 |
| c4-modelleur | Software-architectuur modellering | 2 | 0 | 0 |
| c4-vertaler | C4-modeltransformatie en validatie | 4 | 0 | 0 |
| data-duidingsarchitect | Data-interpretatie en objectpositionering | 5 | 0 | 0 |

## Value Stream: it-development

**Aantal agents**: 2

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| pipeline-executor | Pipeline-uitvoering, workflow-orkestratie | 1 | 0 | 0 |
| workflow-architect | Workflow-ontwerp, multi-agent orkestratie | 3 | 0 | 0 |

## Value Stream: kennispublicatie

**Aantal agents**: 5

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| artikel-schrijver | Artikelproductie, kennisoverdracht | 6 | 0 | 0 |
| de-schrijver | Narratieve tekstproductie, kennisoverdracht | 0 | 0 | 0 |
| essayist | Essayproductie, reflectieve kennisoverdracht | 1 | 0 | 0 |
| heraut | Canonieke aankondiging, governance communicatie | 2 | 0 | 0 |
| vertaler | Tekstvertaling, meertalige kennisoverdracht | 1 | 0 | 0 |

## Value Stream: markt-en-investeringsvorming

**Aantal agents**: 1

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| mandarin-ea | Enterprise Architecture & Strategie | 1 | 0 | 0 |

## Value Stream: utility

**Aantal agents**: 8

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| docker-steward | Docker, containers en lokale C4-visualisatie | 1 | 0 | 0 |
| formaat-vertaler | Formaat-conversie voor documenten | 3 | 0 | 0 |
| kort-schrijver | Tekstoptimalisatie voor interne communicatie | 8 | 0 | 0 |
| layout-optimizer | Diagram-layout en visualisatie-optimalisatie | 1 | 0 | 0 |
| moeder | Workspace-ordening, governance, agent-lifecycle | 11 | 0 | 0 |
| presentatie-architect | Presentatie-ontwerp (design-assets voor publicatie) | 1 | 0 | 0 |
| publicatie-agent | Publicatie en distributie (GitHub Pages) | 1 | 0 | 0 |
| python-expert | Python-ontwikkeling, code-kwaliteit | 6 | 0 | 0 |

## Value Stream: veranderverkenning

**Aantal agents**: 2

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| hypothese-vormer | Productontwikkeling – Verandering & Verkenning | 3 | 0 | 0 |
| niam-analist | Informatieanalyse en conceptuele modellering | 5 | 0 | 0 |

## Value Stream: werkvoorbereiding

**Aantal agents**: 2

| Agent | Domein | Prompts | Runners | Templates |
|-------|--------|---------|----------|-----------|
| thema-verwoorder | themavorming naar epic-statement | 2 | 0 | 1 |
| verbeteringen-beschrijver | expliciete werkvoorbereiding volgens SAFe (THEMA, VERBETERING, WERKTAAK) | 2 | 0 | 0 |

## Metadata

- **Gescande folders**:
  - `agent-charters/` (utility & agent-enablement)
  - `exports/*/charters/` (value streams)
  - `exports/*/charters-agents/` (value streams)
  - `.github/prompts/` (prompts)
  - `exports/*/prompts/` (value stream prompts)
  - `scripts/runners/` (runners)
- **Value stream bron**: Charter header (`**Value Stream**:` veld)
- **Digest**: 5-karakter SHA-256 hash van agents-lijst (gesorteerd) voor change-tracking
- **Traceability**: Agent Curator charter, publiceer-agents-overzicht prompt
