# Mandarin Agent Prompt Architectuur

Dit document beschrijft de werkwijze en architectuurprincipes achter het Mandarin agent prompt systeem.
...
- Alleen wat `run_prompt.py` nodig heeft: `agent`, `intent`, `value_stream`, `bootstrap`
...
**Voordeel**: Procesverbeteringen (zoals run_prompt.py optimaliseren) vereisen geen wijzigingen in governance documenten.
...
**Conventies**:
- Workspace configuratie: **altijd** `beleid-workspace.md` in root (hardcoded in `run_prompt.py`)
...
```
┌─────────────────────────────────────────────────────────────┐
│ 1. run_prompt.py leest beleid-workspace.md                 │
│    ↓                                                        │
...
│    ↓                                                        │
│ 7. run_prompt.py vervangt placeholders in agent-instructies│
│    ↓                                                        │
│ 8. Agent ontvangt volledige context en kan werken          │
└─────────────────────────────────────────────────────────────┘
```
...
Ja, maar **not recommended**. De fallback-mechanisme in `run_prompt.py` ondersteunt dit:
```yaml
bootstrap:
...
- [scripts/run_prompt.py](../scripts/run_prompt.py) - Prompt runner implementatie
- [templates/agent-charter.template.md](../templates/agent-charter.template.md) - Charter template

---

*Laatste update: 2026-02-13*  
*Architectuur versie: 2.0 (Minimale prompts + beleid-workspace.md)*
