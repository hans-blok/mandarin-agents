...
1. **Analyseren (Eénmalig)**:
   - Lees boundary en identificeer alle intents.
   - Bepaal de doelfolder structuur.
2. **Genereren (Intern)**:
   - Stel voor **elke** intent het bijbehorende template op in het geheugen.
   - Stel de README-templates.md op.
3. **Uitvoeren (Batch output)**:
   - Maak de doelfolder aan indien nodig.
   - Schrijf **alle** template-bestanden en de README in één keer weg.
   - Log de acties.

### Kwaliteitsborging
- Elk intent uit de boundary heeft een corresponderend template-bestand.
- Templatenamen volgen het patroon `{agent}.{intent}.template.md`.
- De README legt uit hoe de templates te gebruiken.
