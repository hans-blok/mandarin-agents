...
2. **Agent-boundaries bepalen en vastleggen**  
	 Bepaalt, op basis van aanleiding, gewenste capability en value stream, scherpe agent-boundaries, formuleert de verplichte 4-regel output en legt boundaries vast op een traceerbare plek voor gebruik door Agent Smeder.
	 Bij een nieuwe agent zorgt de agent-curator ervoor dat de per-agentfolder onder `artefacten/` bestaat (patroon: `<value-stream>.<fase>.<agent-naam>/`, bijvoorbeeld `artefacten/aeo.02.agent-curator/` voor deze agent) en plaatst daar het artefact dat de boundary beschrijft als startpunt voor verdere artefacten (charter, contract, prompt).
...
- `artefacten/<value-stream>.<fase>.<agent-naam>/agent-boundary-<agent-naam>.md` (boundary-artefact in de per-agentfolder, bijvoorbeeld `artefacten/aeo.02.agent-curator/agent-boundary-agent-curator.md`)
