...
input_files:
  ...
  template_file: "artefacten/aeo/aeo.02.agent-smeder/templates/{template-naam}.template.md"
  # Veelvoorkomende input bestanden (pas aan naar behoeften van je intent):
  # contract_file: "artefacten/{vs}/{vs}.{fase}.{agent}/agent-contracten/{agent}.{intent}.agent.md"
...
4. input_files sectie
   ...
   - template_file is meestal verplicht: verwijst naar het template dat de agent gebruikt
   - Voorbeeld:
     ```yaml
     input_files:
       template_file: "artefacten/aeo/aeo.02.agent-smeder/templates/agent-contract-intent.template.md"
       boundary_file: "artefacten/{vs}/{vs}.{fase}.{agent}/{agent}.agent-boundary.md"
       contracts_folder: "artefacten/{vs}/{vs}.{fase}.{agent}/agent-contracten/"
     ```
   - Command-line overschrijven blijft mogelijk: -p boundary_file=/ander/pad
   - Commentaar uitgeschakelde opties met # voor optionele bestanden
