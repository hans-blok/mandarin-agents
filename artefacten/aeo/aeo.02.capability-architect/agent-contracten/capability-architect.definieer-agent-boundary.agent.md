...
- agent_doel: Korte beschrijving van het doel van de agent (type: string, 1-2 zinnen).

**Optionele parameters**:
- boundary_draft: Conceptversie van de boundary indien aanwezig (type: string).
- value_stream: Value stream waarin de agent opereert (type: string, bijv. "aeo", "fnd").
- fase: Fase binnen de value stream (type: string, bijv. "01", "02").
- domein: Domein of kennisgebied van de agent (type: string).
- referentie_agents: Lijst van agents met mogelijke raakvlakken (type: list[string]).
...
- stopt wanneer agent_naam of agent_doel ontbreekt;
- stopt wanneer agent_naam niet voldoet aan naamgevingsconventies (kebab-case);
- vraagt om verduidelijking wanneer agent_doel te vaag of te breed is (>3 zinnen);
...
### Stappen
1. **Analyseer input**: Begrijp agent_doel en domein
...
5. **Positioneer in ecosysteem**: Bepaal value stream en fase indien niet opgegeven
6. **Stel intents voor**: Voorlopige lijst van 1-3 intents
7. **Schrijf boundary document**: Volgens template-structuur
...
- ✓ Gelezen bestanden: agent_doel, boundary_draft (indien opgegeven), referentie_agents
...
- STOP: bij te vage agent_doel die niet te scherpstellen is, bij ontbrekende basale informatie
